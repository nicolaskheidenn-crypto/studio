...
export default function DashboardPage() {
  ...
  const handleDispatchWin = () => {
    ...
    setActiveTab('hub'); // Stay/Go to Hub
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fdfaf6]">
      ...
      {/* Dynamic Stats Bar */}
      <div className="bg-white/80 border-b border-foreground/10 backdrop-blur-md sticky top-16 z-40">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          <TabsList className="bg-white/50 p-1.5 rounded-full w-fit shadow-md border border-foreground/10 overflow-x-auto scrollbar-hide">
          ...
          <TabsContent value="hub" className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4">
            <div className="lg:col-span-2 space-y-8">
              {/* Win Dispatcher */}
              <Card className="rounded-[2.5rem] border-foreground/10 shadow-xl p-8 bg-white space-y-6">
                <div className="flex gap-4">
                  ...
                  <Textarea 
                    ...
                    className="flex-1 bg-secondary/30 border-2 border-foreground/10 rounded-3xl p-6 text-base font-medium min-h-[120px]"
                  />
                </div>
                {postImages.length > 0 && (
              ...
              <div className="space-y-8">
                {newsPosts.map((news) => (
                  <Card key={news.id} className="rounded-[3rem] border-primary/40 bg-primary/5 overflow-hidden shadow-lg border-2">
                    ...
                    <CardContent className="p-8 pt-0 space-y-6">
                       ...
                       <p className="text-lg font-medium text-foreground leading-relaxed">{news.content}</p>
                    </CardContent>
                  </Card>
                ))}

                {activityWall.map((post) => (
                  <Card key={post.id} className="rounded-[3rem] border-foreground/10 bg-white shadow-xl overflow-hidden">
                    <CardHeader className="p-8 pb-4">
                    ...
                    <CardContent className="p-8 pt-0 space-y-6">
                       <p className="text-base font-medium text-foreground leading-relaxed">{post.description}</p>
                       ...
                       <div className="flex gap-4 pt-6 border-t border-foreground/10">
                          <Button variant="ghost" className="text-[10px] font-black uppercase tracking-widest text-foreground/60"><Heart className="h-4 w-4 mr-2" /> Respect</Button>
                          <Button variant="ghost" className="text-[10px] font-black uppercase tracking-widest text-foreground/60"><MessageCircle className="h-4 w-4 mr-2" /> Insight</Button>
                       </div>
                    </CardContent>
                  </Card>
                ))}
            ...
            <div className="space-y-8">
              <Card className="rounded-[3rem] border-4 border-primary/20 bg-white p-10 text-center space-y-8 shadow-2xl">
                 ...
                 <div className="p-8 bg-secondary/30 rounded-3xl border-2 border-foreground/10">
          ...
          <TabsContent value="shooppy" className="animate-in fade-in slide-in-from-bottom-4 space-y-12">
             <div className="text-center space-y-2">
                <h3 className="text-5xl font-black text-foreground uppercase tracking-tighter italic">Sovereign Marketplace</h3>
                <p className="text-[10px] font-black uppercase text-primary tracking-widest">Master level ebooks & templates</p>
             </div>
             
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {shooppyProducts.length === 0 ? (
                  <div className="col-span-full py-24 text-center bg-white rounded-[3rem] border-4 border-dashed border-foreground/10">
                  ...
                  <Card key={p.id} className="rounded-[3rem] border-2 border-foreground/5 bg-white shadow-xl overflow-hidden group hover:border-primary transition-all">
                    ...
                    <div className="p-10 space-y-6">
                       ...
                       <p className="text-sm font-medium text-foreground leading-relaxed line-clamp-3">{p.description}</p>
          ...
          <TabsContent value="resources" className="space-y-12 animate-in fade-in slide-in-from-bottom-4">
             <Card className="rounded-[3rem] border-2 border-foreground/10 bg-white p-10 space-y-10 shadow-xl">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                   <div className="space-y-8">
                      <h3 className="text-5xl font-black text-foreground uppercase tracking-tighter italic">Library Labs</h3>
                      <p className="text-lg font-medium text-foreground">Contribute strategic prompts or tips to the collective sovereign library.</p>
                      <div className="space-y-4">
                         ...
                         <div className="space-y-2">
                            <Label>Stream</Label>
                            <select className="w-full h-14 bg-secondary/30 border-2 border-foreground/10 rounded-2xl px-6 font-black uppercase text-foreground" value={resType} onChange={e => setResType(e.target.value as any)}>
                         ...
                         <div className="space-y-2">
                            <Label>Content</Label>
                            <Textarea placeholder="Content (URL or Text)" value={resContent} onChange={e => setResContent(e.target.value)} className="min-h-[120px] bg-secondary/30 border-2 border-foreground/10 rounded-[2rem] p-6" />
                         </div>
                      ...
                      <Tabs defaultValue="ai" className="w-full">
                         ...
                         <TabsContent value="ai" className="space-y-4">
                            {resources.filter(r => r.type === 'AI_Prompt').map(r => (
                              <div key={r.id} className="p-6 bg-secondary/30 rounded-3xl border-2 border-foreground/5">
                                 <h4 className="font-black text-foreground uppercase text-sm">{r.title}</h4>
                                 <p className="text-[10px] text-foreground font-black uppercase mb-3 opacity-60">By @{r.nickname}</p>
                                 <Button variant="outline" className="h-8 rounded-full text-[8px] uppercase font-black px-4 border-foreground">Copy Prompt</Button>
                              </div>
                            ))}
                         </TabsContent>
                         ...
                         <TabsContent value="webin" className="space-y-4">
                            {resources.filter(r => r.type === 'WeBin').map(r => (
                              <div key={r.id} className="p-6 bg-primary/10 rounded-3xl border-2 border-primary/20 flex justify-between items-center">
                                 <div>
                                   <h4 className="font-black text-foreground uppercase text-sm">{r.title}</h4>
                                   <p className="text-xs text-foreground opacity-70">{r.description}</p>
                                 </div>
          ...
          <TabsContent value="faq" className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4">
             <Card className="rounded-[4rem] border-2 border-foreground/10 bg-white p-12 space-y-12 shadow-2xl">
                <div className="text-center space-y-2">
                ...
                <Accordion type="single" collapsible className="w-full space-y-4">
                  {faqs.map((f) => (
                    <AccordionItem key={f.id} value={f.id} className="border-none bg-secondary/30 rounded-[2.5rem] px-10 overflow-hidden transition-all data-[state=open]:shadow-lg">
                      ...
                      <AccordionContent className="text-sm text-foreground leading-relaxed font-medium pb-10 pt-2 border-t border-foreground/10">
                        {f.answer}
                      </AccordionContent>
                    </AccordionItem>
                  ))}
          ...
          <TabsContent value="metext" className="animate-in fade-in slide-in-from-bottom-4">
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[700px]">
                <Card className="rounded-[3rem] border-2 border-foreground/10 bg-white overflow-hidden flex flex-col shadow-xl">
                   <div className="p-8 border-b-2 border-foreground/10 space-y-4">
                   ...
                   <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-hide">
                      <div className="p-6 bg-primary/5 rounded-3xl border-2 border-primary/20 flex items-center justify-between group cursor-pointer">
                         <div className="flex items-center gap-4">
                ...
                <Card className="lg:col-span-2 rounded-[3rem] border-2 border-foreground/10 bg-white overflow-hidden flex flex-col shadow-xl relative">
                   <div className="p-8 border-b-2 border-foreground/10 flex items-center justify-between bg-white sticky top-0 z-10">
                      <div className="flex items-center gap-4">
                   ...
                   <div className="flex-1 overflow-y-auto p-10 space-y-8 bg-secondary/10 scrollbar-hide">
                      <div className="flex gap-4">
                         ...
                         <div className="max-w-[70%] p-6 bg-white rounded-r-[2rem] rounded-bl-[2rem] shadow-md border-2 border-foreground/5">
                            <p className="text-sm font-medium text-foreground leading-relaxed">Welcome to MeText, Strategist. Every great empire is built on clear communication. Use this space to align your vision.</p>
                         </div>
                      </div>
                   </div>
                   <div className="p-8 bg-white border-t-2 border-foreground/10 flex gap-4">
      ...
      <Dialog open={showDaily} onOpenChange={setShowDaily}>
        <DialogContent className="rounded-[4rem] border-8 border-primary/20 bg-white/95 backdrop-blur-xl p-16 max-w-sm text-center shadow-2xl">
          <div className="space-y-10">
            ...
            <div className="p-8 bg-secondary/50 rounded-[2.5rem] border-2 border-foreground/5 flex justify-around">
        ...
        <DialogContent className="rounded-[4rem] border-8 border-primary/20 bg-white/95 backdrop-blur-xl p-16 max-w-md shadow-2xl">
          ...
          <div className="space-y-6 mt-10">
            {[
              ...
              <div key={r.lv} className={cn("p-8 rounded-[2.5rem] border-4 flex items-center justify-between", level >= r.lv ? "bg-primary/10 border-primary" : "bg-foreground/5 border-foreground/20 opacity-40")}>
                ...
                <ShieldCheck className={cn("h-8 w-8", level >= r.lv ? "text-primary" : "text-foreground/30")} />
              </div>
            ))}
          </div>
        </DialogContent>
